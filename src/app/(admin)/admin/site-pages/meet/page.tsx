...
import { MeetPageForm } from "@/components/admin/MeetPageForm";
...
export default async function AdminMeetPage() {
  const config = await prisma.pageConfig.findUnique({ where: { slug: "meet" } });
  const raw = (config?.data ?? {}) as Partial<MeetData>;

  const initial: MeetData = {
  ...
  return (
    <div>
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-stone-900">Meet page</h1>
        <p className="text-sm text-stone-400 mt-0.5">
          Edit the hero, add sections (people grids, stats, chapters, CTAs), and toggle visibility per section
        </p>
      </div>
      <MeetPageForm initial={initial} />
    </div>
  );
}
